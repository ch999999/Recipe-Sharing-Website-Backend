...
namespace RecipeSiteBackend.Services
{
    public class RecipeService
    {
            ...
            try
            {
                ...
                if (recipeInstructionImages.Count > 0)
                {
                    foreach (var image in recipeInstructionImages)
                    {
                        ...
                        awsToDelete.Add(url);
                        //var head = "https://" + _configuration["AWS:AWS_BUCKET"] + ".s3." + _configuration["AWS:AWS_REGION"] + ".amazonaws.com/";
                        //var key = url.Substring(url.IndexOf(head) + head.Length);
                        //await client.DeleteObjectAsync(new Amazon.S3.Model.DeleteObjectRequest() {BucketName = _configuration["AWS:AWS_BUCKET"], Key = key });
                    }
                }
                var descriptionImage = await getDescription_Media(recipe.UUID);
        ...
        public bool checkIfImageUsed(string url)
        {
            ...
            //var a = _context.Description_Medias.Where(dm=>dm.Url== url).AsNoTracking();
            var b = _context.Instruction_Images.AsNoTracking().FirstOrDefault(im => im.Url == url);
            //var b = _context.Instruction_Images.Where(im => im.Url == url).AsNoTracking();
            if (a != null || b != null)
            {
                return true;
            }
